...
export async function get_latest_news({ topic, language = "en" }) {
  try {
    ...
    console.error('Error in get_latest_news:', error.message);
...
export async function get_google_places({
  ...
  try {
    ...
    console.error('Error in get_google_places:', error.message);
    return { error: `Failed to fetch places: ${error.message}` };
  }
}
